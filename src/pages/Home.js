...
export default function LandingPage() {
   // Supprimer ces lignes en double
    // const [isMenuOpen, setIsMenuOpen] = useState(false);
    // const [showLoginModal, setShowLoginModal] = useState(false);
    
    // Garder uniquement ces états
    const [mobileMenuOpen, setMobileMenuOpen] = useState(false);
    const [showLoginModal, setShowLoginModal] = useState(false);
    const pricingRef = useRef(null);
    const faqRef = useRef(null);

    // Supprimer cette fonction qui utilise isMenuOpen
    // const closeMenu = () => {
    //   setIsMenuOpen(false);
    // };
    ...
    const scrollToSection = (ref) => {
        ...
        setMobileMenuOpen(false); // Utiliser mobileMenuOpen au lieu de closeMenu
    };

    const handleAuth = (type) => {
        if (type === 'logins') { // Changer 'logins' en 'login'
            setShowLoginModal(true);
        } else {
            window.location.href = `/${type}`;
        }
        setMobileMenuOpen(false); // Fermer le menu mobile après l'action
    };
  // Fonction pour fermer le menu

//   const scrollToSection = (ref) => {
//     ref.current?.scrollIntoView({ behavior: 'smooth' });
//     closeMenu(); // Ferme le menu après le scroll
//   };
...
//       window.location.href = `/${type}`;
//     }
//     closeMenu(); // Ferme le menu après l'action d'authentification
//   };
